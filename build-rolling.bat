...
DEL /F /Q /A "%~dp0container.txt"
docker system prune -a -f
docker build https://raw.githubusercontent.com/randomtable/ChimeraOS/master/DockerFile-Tor-obfs4-bridge --iidfile "%~dp0image.txt"
SET /P iid=<"%~dp0image.txt"
SET iid=%iid:sha256:=%
docker create %iid%
docker ps -l -q >> container.txt
SET /P ciid=<"%~dp0container.txt"
docker commit %ciid% alessandrofiori/tor-obfs4-bridge:latest
docker login
docker push alessandrofiori/tor-obfs4-bridge:latest
